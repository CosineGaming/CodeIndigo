...
void update(int time)
{
	...
	{
		gravity = 0;
		camera.Y = int (camera.Y - 1.5) - (camera.Y > 0) + 1.5;//1.5
	}
...
{
	Direction test(1.0, 93.0, 0.0);
	test.Set_Direction(1.0, test.Get_X_Angle(), 0.0);
	std::cout << test.Get_X_Angle() << ":" << test.Get_X() << "," << test.Get_Y() << "," << test.Get_Z() << "," << std::endl;
	...
	srand(time(0));
	for (int cube=0; cube<10000; ++cube)
	{
