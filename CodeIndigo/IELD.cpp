...
bool Add_And_Save(glm::vec3& position, const std::string& mesh, const std::string& texture)
{
	...
	if (location == model_to_index.end())
	{
		...
		std::cout << together.mesh << ", " << together.texture << std::endl;
		Indigo::Current_World.Add_Object(Object(position.x, position.y, position.z, add));
	}
	else
	{
		std::cout << "Green!\n";
		Indigo::Current_World.Add_Object(Object(position.x, position.y, position.z, meshes[location->second]));
...
void Mouse_Interact(int button, int state, float x, float y)
{
	if (state == GLFW_RELEASE)
	{
		...
		{
			To_Move->Data.Color /= glm::vec4(5, 5, 5, 1);
