...
bool Save(const char * filename)
{
	...
	file << Indigo::Current_World.View.X << "|" << Indigo::Current_World.View.Y << "|" << Indigo::Current_World.View.Z << "|" << Camera_Speed << "|" << Snap << std::endl;
		...
		if ((key == GLFW_KEY_ENTER || key == GLFW_KEY_KP_ENTER))
		{
			if (menu_stage == 2)
			{
				...
				if (Add_And_Save(position, mesh, texture, bump_map))
				{
					...
					Undo_Object = Object();
					Toggle_Select(&Indigo::Current_World.Get_Object(Undo_Index));
					Indigo::Current_World.Remove_2D_Object(space_menu);
					space_menu = -1;
					Current_Menu = -1;
				}
	...
	else
	{
		if (key == ' ' || (Indigo::Control && key == 'a')) // Add objects
		{
			...
			*name = 0;
			...
			int loading = Indigo::Current_World.Add_2D_Object(Object(menu_x, menu_y));
			...
			if (Add_And_Save(position, mesh, texture, bump_map))
			{
				...
				Undo_Object = Object();
				Toggle_Select(&Indigo::Current_World.Get_Object(Undo_Index));
				Indigo::Current_World.Remove_2D_Object(loading);
			}
			else
			{
				Print(loading, "Failed to load.");
				Indigo::Current_World.Get_2D_Object(loading).Update_Function = Fade_Text;
			}
			Indigo::Keys[' '] = false;
		...
		if (Indigo::Control)
		{
			...
			if (key == 'v') // Paste
			{
				if (Copied != -1)
				{
					...
					if (To_Move == &copied)
					{
						// We copied a selected (*1.8 color) object
						pasted.Color /= 1.8;
					}
